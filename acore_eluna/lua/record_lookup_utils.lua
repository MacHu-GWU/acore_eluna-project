--[[
我们定义一个 record 就是结构体数据. 里面有很多 key, value. 并且这里有且只有一个 key 是 id.
这个 id 可以是 integer 也可以是 string.
...
--]]
---@param recordList table: 所有 record 的列表
---@param idKey string: record 的 id 在哪个 key 下面
---@return table: 满足条件的 record 的列表
